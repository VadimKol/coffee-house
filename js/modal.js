...
for (let child of document.querySelector("#grid").children) {
  //3 сетки кофе, чай и десерт
...
window.closeModal = function () {
  //так как модуль, видимость модуля, нужно функцию сделать глобальной
  if ((isBtnClose && isModal) || (!isModal && !isBtnClose)) {
    //закрываем если кликаем на кнопку в модалке, или не на модалку и не на кнопку
    document.querySelector(".modal").classList.remove("show-modal");
    document.body.classList.remove("disable-scrolling");
    document
      .querySelector(".modal-item")
      .removeEventListener("click", isInModal);
    document
      .querySelector(".modal-item-info__btn-close")
      .removeEventListener("mousedown", isInBtnCLose);
...
function showModal() {
  ...
  document
    .querySelector(".modal-item-info__btn-close")
    .addEventListener("mousedown", isInBtnCLose);

  function toggleSize() {
    document
      .querySelector(".modal-item-info-list-s__btn--active")
      .classList.remove("modal-item-info-list-s__btn--active");
  ...
  function calcTotalPrice() {
    ...
    let activeSizePrice = document
      .querySelector(".modal-item-info-list-s__btn--active")
      .querySelector(".hidden_price").innerText;
    let allActiveAdditives = document.querySelectorAll(
      ".modal-item-info-list-a__btn--active"
    );
    ...
    totalPrice.innerText =
      "$" +
      (Number(basePrice) + Number(activeSizePrice) + allAdditivesPrice).toFixed(
        2
      );
  }
